...
// === 4. BOX_SPLIT과 PASSAGE_SPLIT 처리 함수 ===
export const renderTextWithSplits = (text, splitType = 'PASSAGE_SPLIT') => {
  ...
  return blocks.map((block, index) => {
    // 이미 _processFootnotes가 적용된 텍스트를 받으므로 바로 renderText 적용
    const renderedBlock = renderText(block);
    
    if (splitType === 'PASSAGE_SPLIT') {
      return (
        <div key={index} className={`p-2 border border-gray-200 dark:border-gray-800 rounded bg-transparent ${index > 0 ? 'mt-2' : ''}`}>
          {renderedBlock}
        </div>
      );
    } else if (splitType === 'BOX_SPLIT') {
      return (
        <div key={index} className={`p-3 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded shadow-inner ${index > 0 ? 'mt-2' : ''}`}>
          {renderedBlock}
        </div>
      );
    }
