...
export const renderTextWithSplits = (text, splitType = 'PASSAGE_SPLIT') => {
  ...
  return blocks.map((block, index) => {
    ...
    if (splitType === 'PASSAGE_SPLIT') {
      return (
        <div key={index} className="border-l-4 border-blue-500 pl-4 my-3 bg-blue-50 dark:bg-blue-900/20 py-2">
          {renderedBlock}
        </div>
      );
    } else if (splitType === 'BOX_SPLIT') {
      return (
        <div key={index} className="border border-gray-300 dark:border-gray-600 p-3 my-2 rounded bg-gray-50 dark:bg-gray-800">
          {renderedBlock}
        </div>
      );
    }
